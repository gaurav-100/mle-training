...
def get_feature_names_from_column_transformer(col_trans):
    ...
    being actively worked upon.
        ...
        NOTE: This utility function is a temporary solution
    until the proper fix is available in the `scikit-learn` library.

    - SimpleImputer has `add_indicator` attribute that
    distinguishes it from other transformers
    - Encoder had `get_feature_names` attribute that
    distinguishes it from other transformers
    - The last transformer is ColumnTransformer's 'remainder'
    """
    col_name = []
    for transformer_in_columns in col_trans.transformers:
        is_pipeline = 0
